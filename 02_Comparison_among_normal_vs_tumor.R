...
p <- ggplot(tumor_vs_normal, aes(x=Project, y = ENST00000620340.4, color = Project)) +  # ENST00000262752.4/ ENST00000620340.4
  geom_boxplot() + 
...
p + stat_compare_means(method ="wilcox.test", label.y = 2.3, label.x = 1.5, label = "p.signif") + 
  theme_light() + theme(axis.text.y = element_text(size = 12), axis.title.y = element_text(size = 12),axis.text.x = element_text(size = 12), axis.title.x = element_text(size = 12),plot.title = element_text(size = 16), legend.text = element_text(size = 10), legend.title = element_text(size =12)) + 
